    ...
    <div class="svg-pallet">
      <svg v-for="(rect, index) in rectangles"
        ...
        <g :transform="palletTransform(index)">
          ...
          />
          <!-- Планки поддона -->
          <template v-for="i in 3" :key="`plank-${i}`">
            <rect
                :height="palletHeight"
                :width="20"
                :x="(getPalletWidth(index) / 4) * (i - 0.5)"
                fill="#A0522D"
                y="0"
            />
          </template>
        ...
        <g v-if="index === 0" class="dimension-line">
          ...
          />
          <!-- Стрелки -->
          <path d="M -45 50 L -30 0 L -15 50" fill="#E32029" stroke="#E32029"/>
          <path
              :d="`M -45 ${rect.height + palletHeight - 50} L -30 ${rect.height + palletHeight} L -15 ${rect.height + palletHeight - 50}`"
              fill="#E32029"
              stroke="#E32029"
          />
        ...
        <g class="dimension-line">
          <line
              ...
              :y1="rect.height + palletHeight + 70"
              :y2="rect.height + palletHeight + 70"
              stroke="#E32029"
              stroke-width="3"
              x1="0"
          />
          <!-- Стрелки -->
          <path
              :d="`M 50 ${rect.height + palletHeight + 55} L 0 ${rect.height + palletHeight + 70} L 50 ${rect.height + palletHeight + 85}`"
              fill="#E32029"
              stroke="#E32029"
          />
          <path
              :d="`M ${getMaxWidth(index) -50} ${rect.height + palletHeight + 55} L ${getMaxWidth(index)} ${rect.height + palletHeight + 70} L ${getMaxWidth(index)-50} ${rect.height + palletHeight + 85}`"
              fill="#E32029"
              stroke="#E32029"
          />
          <!-- Текст ширины -->
...
const viewBoxes = computed(() => {
  return rectangles.value.map((_, index) => {
    ...
    return `-100 -50 ${totalWidth} ${totalHeight}`
  })
})
...
.svg-pallet svg {
  min-width: 500px; /* Фиксированная ширина для обоих SVG */
