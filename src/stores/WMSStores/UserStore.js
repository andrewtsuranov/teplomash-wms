      ...
      async (request, options, response) => {
        if (response.status === 401) {
          ...
          errorStore.setError({
            ...
            message:
              "Ошибка авторизации! Пользователь не авторизован, требуется регистрация!",
          });
        }
      },
    ],
...
export const useUserStore = defineStore("userStore", () => {
  ...
  const fullListUsers = ref(
    JSON.parse(localStorage.getItem("fullListUsers")) || null,
  );
  //getters
  ...
  const getFullNameUser = computed(() => {
    ...
    const [lastName, firstName, middleName] =
      user.value.user.username.split("_");
    return {
      lastName,
      firstName,
      middleName,
    ...
    try {
      ...
      if (e.response?.status === 401) {
        errorStore.setError({
          ...
          message:
            "Ошибка авторизации! Проверьте правильность введённых данных или зарегистрируйтесь!",
        });
        ...
        errorStore.setError({
          ...
          message:
            "Ошибка авторизации! Произошла ошибка при загрузке данных, попробуйте позже!",
        });
      }
      throw e;
    } finally {
