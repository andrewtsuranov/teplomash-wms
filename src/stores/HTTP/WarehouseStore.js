...
export const useWarehouseStore = defineStore("warehouseStore", () => {
  ...
  const allWarehouses = ref(
    JSON.parse(localStorage.getItem("warehouses")) || null,
  );
  const warehouseData = ref(
    JSON.parse(localStorage.getItem("warehouseData")) || null,
  );
  const selectedZone = ref(
    JSON.parse(localStorage.getItem("selectedZone")) || null,
  );
  const allZoneTypes = ref(null);
  const warehouseZoneTypeById = ref(
    JSON.parse(localStorage.getItem("warehouseZoneTypeById")) || null,
  );
  const warehouseAliasMap = ref({});
  //getters
  const getWarehouseId = computed(() => warehouseData.value.id);
  const groupByZone = computed(() =>
    useGroupByKey(warehouseZoneTypeById.value, "zone"),
  );
  ...
  const GET_WAREHOUSE_BY_ID = async (id) => {
    ...
    try {
      ...
      const alias = useTransliterate(response.name)
        .toLowerCase()
        .replace(/\s+/g, "-");
